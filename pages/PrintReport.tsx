...
const PrintReport: React.FC = () => {
  ...
  const DataRow = ({ label, value }: { label: string; value: any }) => (
    <div className="flex gap-2 py-1 text-sm border-b border-sand-100 last:border-0">
      <span className="font-bold text-sand-600 min-w-[150px]">{label}:</span>
      <span className="text-sand-900">{value || 'N/A'}</span>
    </div>
  );
  ...
  return (
    <div className="max-w-[800px] mx-auto p-12 bg-white print:p-0 min-h-screen">
      ...
      {/* HEADER MÉDICO */}
      <div className="flex justify-between items-start border-b-4 border-sand-900 pb-6 mb-8">
        <div className="flex gap-4 items-center">
          <div className="flex items-center justify-center bg-white rounded-2xl overflow-hidden shadow-sm border border-sand-100"
            ...
            {settings.logo_url ? (
              ...
              <div className="w-full h-full bg-sand-900 text-white flex items-center justify-center font-black text-2xl">
        ...
        <div className="text-right text-[10px] text-sand-400 font-bold uppercase tracking-tighter">
          <p>Expediente generado el:</p>
          <p className="text-sand-900 text-sm">{new Date().toLocaleDateString('es-ES', { day: '2-digit', month: 'long', year: 'numeric' })}</p>
        </div>
      </div>

      {/* DATOS DEL PACIENTE */}
      <SectionHeader title="Datos del Paciente" />
      <div className="grid grid-cols-2 gap-x-12 bg-sand-50 p-6 rounded-2xl border border-sand-200">
      ...
      {dermHist.length > 0 && (
        <>
          ...
          {dermHist.map((h, idx) => (
            <div key={h.id} className={`space-y-4 ${idx > 0 ? 'mt-8 pt-8 border-t border-dashed border-sand-300' : ''}`}>
              <div className="flex justify-between items-center mb-2">
                <span className="bg-sand-200 px-3 py-1 rounded text-xs font-black">CONSULTA: {new Date(h.fecha).toLocaleDateString()}</span>
                <span className="font-bold text-xs uppercase text-sand-500">Fitzpatrick: {h.tipo_piel_fitzpatrick}</span>
              </div>
              <div className="space-y-3">
                <div>
                  <h4 className="text-xs font-black text-sand-500 uppercase">Motivo de Consulta:</h4>
                  <p className="text-sm font-bold text-sand-800 italic">"{h.motivo_consulta}"</p>
                </div>
                <div className="grid grid-cols-2 gap-8">
                  <div>
                    <h4 className="text-xs font-black text-sand-500 uppercase">Antecedentes Personales:</h4>
                    <p className="text-xs text-sand-700 leading-relaxed">{h.antecedentes_personales_patologicos || 'Negados'}</p>
                  </div>
                  <div>
                    <h4 className="text-xs font-black text-sand-500 uppercase">Antecedentes Familiares:</h4>
                    <p className="text-xs text-sand-700 leading-relaxed">{h.antecedentes_familiares || 'Negados'}</p>
                  </div>
                </div>
                <div>
                  <h4 className="text-xs font-black text-sand-500 uppercase">Evolución y Notas:</h4>
                  <p className="text-xs text-sand-700">{h.evolucion_clinica}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        <>
          ...
          {trichHist.map((th) => (
            <div key={th.id} className="space-y-3 mb-6">
              <span className="bg-sand-200 px-3 py-1 rounded text-xs font-black">ANÁLISIS CAPILAR: {new Date(th.fecha).toLocaleDateString()}</span>
              ...
              <div className="bg-sand-50 p-4 rounded-xl border border-sand-200 text-xs italic">
      ...
      {sessions.length > 0 && (
        <>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-sand-100 text-[10px] font-black uppercase text-sand-600">
                <th className="p-2 border border-sand-200">Fecha</th>
                <th className="p-2 border border-sand-200">Evolución Clínica</th>
                <th className="p-2 border border-sand-200">Ajuste Terapéutico</th>
              </tr>
            </thead>
            <tbody>
              {sessions.map(s => (
                <tr key={s.id} className="text-xs">
                  <td className="p-2 border border-sand-200 font-bold whitespace-nowrap">{new Date(s.fecha).toLocaleDateString()}</td>
                  <td className="p-2 border border-sand-200">{s.evolucion_clinica}</td>
                  <td className="p-2 border border-sand-200 font-bold text-terracotta-800">{s.ajustes_terapeuticos}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="mt-20 flex justify-between items-end">
        ...
        <div className="flex flex-col items-center">
          <div className="w-48 border-b-2 border-sand-900 mb-2"></div>
          <p className="text-xs font-black text-sand-900">{settings.doctor_name}</p>
          <p className="text-[10px] text-sand-500 font-bold">{settings.doctor_profession}</p>
        </div>
      </div>
    </div>
  );
