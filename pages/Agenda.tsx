    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20 max-w-4xl mx-auto">
      ...
      <section className={`${cardClasses} p-6`}>
        <div className="flex items-center justify-between mb-6">
          <button onClick={() => shiftDay(-1)} aria-label="Día anterior" className="p-2 rounded-xl border border-sand-300 hover:bg-sand-50 transition-all">
          ...
          <input
            ...
            className="px-4 py-2 bg-sand-50 border border-sand-300 rounded-xl text-sm font-black text-sand-800"
          />
          <button onClick={() => shiftDay(1)} aria-label="Día siguiente" className="p-2 rounded-xl border border-sand-300 hover:bg-sand-50 transition-all">
        ...
        <div className="space-y-4">
          {dayAppointments.length > 0 ? (
            ...
            <div className="py-16 text-center bg-sand-50 rounded-3xl border-2 border-dashed border-sand-200">
              <p className="text-xs font-black text-sand-400 uppercase tracking-widest">Sin citas para este día</p>
            </div>
          )}
        </div>
      </section>
