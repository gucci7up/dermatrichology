...
const Landing: React.FC = () => {
  ...
  if (showSuccess) {
    return (
      <div className="min-h-screen bg-sand-50 flex items-center justify-center p-6 animate-in fade-in duration-700">
        <div className="max-w-md w-full bg-white rounded-[3rem] shadow-2xl p-10 text-center border border-sand-100">
          ...
          <button
            ...
            className="w-full bg-sand-900 text-white py-4 rounded-2xl font-black shadow-xl hover:bg-black transition-all"
          >
            VOLVER AL INICIO
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-white text-sand-900 font-sans selection:bg-[#C15F3C]/30 pb-20 scroll-smooth">
      {/* Header Fijo */}
      <header className="fixed top-0 left-0 right-0 bg-white/80 backdrop-blur-md border-b border-sand-100 z-50">
        <div className="max-w-7xl mx-auto px-6 h-28 flex items-center justify-center md:justify-between">
          <div className="flex items-center gap-3">
            {settings.logo_url ? (
              ...
              <>
                <div className="w-12 h-12 bg-[#C15F3C] rounded-xl flex items-center justify-center text-white font-black text-2xl">{settings.app_name?.[0] || 'D'}</div>
                <span className="font-black text-2xl tracking-tighter">{settings.app_name || 'DermaTrich'}</span>
              </>
            )}
          </div>
          <nav className="hidden md:flex items-center gap-8 text-xs font-black uppercase tracking-widest text-sand-500">
            <a href="#servicios" className="hover:text-sand-900 transition-colors">Servicios</a>
            <button
              onClick={scrollToBooking}
              className="bg-sand-900 text-white px-8 py-3 rounded-full hover:scale-105 transition-all shadow-lg active:scale-95"
            >
              CITAS
            </button>
          </nav>
      ...
      <section className="pt-48 pb-20 px-6">
        <div className="max-w-5xl mx-auto text-center">
          ...
          <p className="text-lg text-sand-500 font-medium max-w-2xl mx-auto">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {services.map((service, idx) => (
              <div key={idx} className="p-8 bg-sand-50 rounded-[2.5rem] border border-sand-100 hover:border-[#C15F3C]/30 hover:bg-white hover:shadow-xl transition-all group flex flex-col items-center text-center">
                <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center text-[#C15F3C] mb-6 shadow-sm group-hover:scale-110 group-hover:bg-[#C15F3C] group-hover:text-white transition-all duration-300">
                  <service.icon className="w-7 h-7" />
                </div>
                <h4 className="font-black text-sand-900 text-sm leading-tight uppercase tracking-tight">{service.name}</h4>
              </div>
            ))}
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <button
                ...
                className={`p-6 rounded-3xl border-2 flex flex-col items-center gap-3 transition-all ${formData.especialidad === 'derm' ? 'border-[#C15F3C] bg-[#C15F3C]/5 ring-4 ring-[#C15F3C]/10' : 'border-sand-100 hover:border-sand-200'}`}
              >
                <Stethoscope className={`w-8 h-8 ${formData.especialidad === 'derm' ? 'text-[#C15F3C]' : 'text-sand-300'}`} />
                <span className="font-black text-sm uppercase">Dermatología</span>
              </button>
              <button
                type="button"
                onClick={() => setFormData({ ...formData, especialidad: 'trich' })}
                className={`p-6 rounded-3xl border-2 flex flex-col items-center gap-3 transition-all ${formData.especialidad === 'trich' ? 'border-sand-800 bg-sand-50 ring-4 ring-sand-100' : 'border-sand-100 hover:border-sand-200'}`}
              >
                <Scissors className={`w-8 h-8 ${formData.especialidad === 'trich' ? 'text-sand-800' : 'text-sand-300'}`} />
            ...
            <div className="space-y-6">
              <div className="relative">
                <User className="absolute left-5 top-5 w-5 h-5 text-sand-300" />
                <input
                  type="text"
                  required
                  ...
                  className="w-full pl-14 pr-6 py-5 bg-sand-50 border border-sand-200 rounded-2xl focus:ring-2 focus:ring-[#C15F3C] outline-none font-bold text-sand-800 placeholder-sand-400 transition-all shadow-sm"
                />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="relative">
                  <Phone className="absolute left-5 top-5 w-5 h-5 text-sand-300" />
                  <input
                    type="tel"
                    required
                    ...
                    className="w-full pl-14 pr-6 py-5 bg-sand-50 border border-sand-200 rounded-2xl focus:ring-2 focus:ring-[#C15F3C] outline-none font-bold shadow-sm"
                  />
                </div>
                <div className="relative">
                  <Mail className="absolute left-5 top-5 w-5 h-5 text-sand-300" />
                  <input
                    type="email"
                    required
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="relative">
                  <Calendar className="absolute left-5 top-5 w-5 h-5 text-sand-300" />
                  <input
                    type="date"
                    required
                    value={formData.fecha_preferida}
                    onChange={e => setFormData({ ...formData, fecha_preferida: e.target.value })}
                    className="w-full pl-14 pr-6 py-5 bg-sand-50 border border-sand-200 rounded-2xl focus:ring-2 focus:ring-[#C15F3C] outline-none font-bold text-sand-400 focus:text-sand-800 shadow-sm"
                  />
                  <label className="absolute -top-2 left-4 px-2 bg-white text-[10px] font-bold text-sand-400 uppercase tracking-wider">Fecha Preferida</label>
                </div>
                <div className="relative">
                  <Clock className="absolute left-5 top-5 w-5 h-5 text-sand-300" />
                  <input
                    type="time"
                    required
                    ...
                    className="w-full pl-14 pr-6 py-5 bg-sand-50 border border-sand-200 rounded-2xl focus:ring-2 focus:ring-[#C15F3C] outline-none font-bold text-sand-400 focus:text-sand-800 shadow-sm"
                  />
                  <label className="absolute -top-2 left-4 px-2 bg-white text-[10px] font-bold text-sand-400 uppercase tracking-wider">Hora Preferida</label>
                </div>
              </div>

              <div className="relative">
                <Baby className="absolute left-5 top-5 w-5 h-5 text-sand-300" />
                <input
                  type="date"
                  ...
                  className="w-full pl-14 pr-6 py-5 bg-sand-50 border border-sand-200 rounded-2xl focus:ring-2 focus:ring-[#C15F3C] outline-none font-bold text-sand-400 focus:text-sand-800 shadow-sm"
                />
                <label className="absolute -top-2 left-4 px-2 bg-white text-[10px] font-bold text-sand-400 uppercase tracking-wider">Fecha de Nacimiento (Opcional)</label>
              </div>
              ...
              <textarea
                ...
                className="w-full px-6 py-5 bg-sand-50 border border-sand-200 rounded-2xl focus:ring-2 focus:ring-[#C15F3C] outline-none font-bold h-32 resize-none shadow-sm"
              />
            </div>
            ...
            <button
              ...
              className="w-full bg-sand-900 text-white py-6 rounded-2xl font-black text-lg shadow-2xl hover:bg-black transition-all flex items-center justify-center gap-3 disabled:opacity-50 active:scale-95"
            >
