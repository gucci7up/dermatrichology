  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard label="Pacientes Totales" value={totalPatients} icon={Users} trend={0} color="bg-[#C15F3C]" />
          ...
          <div className="bg-white rounded-2xl border border-sand-200 shadow-md overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-sand-50 text-sand-400 uppercase text-[10px] font-bold tracking-wider">
                  <tr>
                ...
                <tbody className="divide-y divide-sand-200">
                  {patients.length > 0 ? patients.map((patient) => (
                    <tr key={patient.id} className="hover:bg-sand-50/80 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <img src={patient.foto_perfil} className="w-10 h-10 rounded-full object-cover" alt={`Foto de ${patient.nombre_completo}`} />
                          <div>
                            <p className="font-semibold text-sand-800">{patient.nombre_completo}</p>
                            <p className="text-xs text-sand-500 capitalize">{patient.sexo === 'M' ? 'Masculino' : 'Femenino'}, {patient.fecha_nacimiento}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-sm text-sand-600 font-mono">{patient.documento_identidad}</td>
                      <td className="px-6 py-4 text-sm text-sand-500">{patient.created_at ? new Date(patient.created_at).toLocaleDateString() : 'N/A'}</td>
                      <td className="px-6 py-4">
                        <Link to={`/patients/${patient.id}`} className="p-2 text-sand-400 hover:text-[#C15F3C] transition-colors inline-block">
                    ...
                    <tr>
                      <td colSpan={4} className="px-6 py-10 text-center text-sand-400">No hay pacientes registrados recientemente.</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          <div className="bg-white rounded-2xl border border-sand-200 shadow-md p-6">
            ...
            <div className="space-y-4">
              {appointments.length > 0 ? appointments.map(app => (
                <div key={app.id} className="p-4 bg-sand-50 rounded-xl border border-sand-200 group hover:border-indigo-300 transition-all">
                  <div className="flex justify-between items-start mb-2">
                    <p className="font-black text-sand-900 text-sm">{app.paciente_nombre}</p>
                    <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded ${app.especialidad === 'derm' ? 'bg-terracotta-100 text-terracotta-700' : 'bg-orange-100 text-orange-700'}`}>
                      {app.especialidad === 'derm' ? 'Derma' : 'Trich'}
                    </span>
                  </div>
                  <p className="text-xs text-sand-500 mb-3 line-clamp-2 italic">"{app.motivo}"</p>
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-bold text-sand-400 flex items-center gap-1">
                ...
                <p className="text-center py-8 text-sand-400 text-sm">No hay solicitudes nuevas.</p>
              )}
            </div>
            <Link to="/booking" className="w-full mt-6 py-2.5 rounded-xl border-2 border-sand-100 text-sand-600 font-semibold text-xs hover:bg-sand-50 transition-all flex items-center justify-center gap-2">
